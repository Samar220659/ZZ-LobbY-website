...
# Import tax compliance system
from tax_compliance import init_tax_system, get_tax_system
...
@api_router.get("/tax/compliance")
async def get_tax_compliance_status():
    """Hole aktuellen Steuerlichen Compliance Status"""
    try:
        tax_sys = get_tax_system()
        if not tax_sys:
            raise HTTPException(status_code=500, detail="Tax system not initialized")
        compliance_status = await tax_sys.get_tax_compliance_status()
        return {"success": True, "compliance": compliance_status}
    except Exception as e:
        logging.error(f"Tax Compliance Status Fehler: {e}")
        raise HTTPException(status_code=500, detail="Failed to get tax compliance status")

@api_router.post("/tax/create-invoice")
async def create_tax_invoice(sale_data: dict):
    ...
    try:
        ...
        # Create invoice
        invoice = await tax_sys.create_invoice(sale_data)
        
        # Generate PDF content
        pdf_content = tax_sys.generate_invoice_pdf_content(invoice)
        ...
        logging.error(f"Create Invoice Fehler: {e}")
        raise HTTPException(status_code=500, detail="Failed to create invoice")

@api_router.get("/tax/monthly-summary/{year}/{month}")
async def get_monthly_tax_summary(year: int, month: int):
    """Hole monatliche Steuer-Zusammenfassung"""
    try:
        tax_sys = get_tax_system()
        if not tax_sys:
            raise HTTPException(status_code=500, detail="Tax system not initialized")
        summary = await tax_sys.get_monthly_tax_summary(year, month)
        return {"success": True, "summary": summary}
    except Exception as e:
        logging.error(f"Monthly Tax Summary Fehler: {e}")
        raise HTTPException(status_code=500, detail="Failed to get monthly tax summary")

@api_router.get("/tax/elster-export/{year}/{month}")
async def get_elster_export(year: int, month: int):
    """Generiere Elster Export für USt-Voranmeldung"""
    try:
        tax_sys = get_tax_system()
        if not tax_sys:
            raise HTTPException(status_code=500, detail="Tax system not initialized")
        elster_data = await tax_sys.generate_elster_export(year, month)
        return {"success": True, "elster_export": elster_data}
    except Exception as e:
        logging.error(f"Elster Export Fehler: {e}")
        raise HTTPException(status_code=500, detail="Failed to generate Elster export")

@api_router.get("/tax/yearly-summary/{year}")
async def get_yearly_tax_summary(year: int):
    """Hole jährliche Steuer-Zusammenfassung"""
    try:
        tax_sys = get_tax_system()
        if not tax_sys:
            raise HTTPException(status_code=500, detail="Tax system not initialized")
        summary = await tax_sys.get_yearly_tax_summary(year)
        return {"success": True, "summary": summary}
    except Exception as e:
        logging.error(f"Yearly Tax Summary Fehler: {e}")
        raise HTTPException(status_code=500, detail="Failed to get yearly tax summary")
